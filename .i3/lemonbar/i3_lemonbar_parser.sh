...
while read -r line ; do
  case $line in
    ...
    GMA*)
      ...
      #gmail="%{F${color_fore}}${sep_l_left}%{F${mail_cback}}%{F${sep_left}${mail_cicon} B${mail_cback}} %{T2}${icon_mail}%{F${mail_cfore} T1} ${gmail}"
  ...
  # And finally, output
  printf "%s\n" "%{l}${wsp}${title} %{r}${mpd}${stab}${gmail}${stab}${cpu}${stab}${mem}${stab}${ethd}${stab}${ethu}${stab}${vol}${stab}${date}${stab}${time}"
  #printf "%s\n" "%{l}${wsp}${title}"
done
